...
- Backend tests use the standard `testing` package. Service tests usually run against in-memory SQLite through the domain's `newTestDB`/`newTestService` helpers; injected seams are faked where a DB-backed check is impractical (`model.Notifier`, `model.FileRemover`, `mailer.SetMailCaptureHook`) — `page` uses a fake `Repository`.
