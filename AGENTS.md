...
- Package dependency rules (acyclic graph):
  ...
  - `cache/` is a leaf package providing the cache backends: an in-process memory implementation and a Valkey (Redis-compatible) implementation built on `valkey-io/valkey-go` (keys are namespaced with a `vexgo:` prefix). It defines one `Cache` interface (`Get`/`Set`/`Delete`/`GetDel`/`Incr` + `Close`); consumers never import it — they declare their own narrow seams (e.g. `middleware.CounterStore`, `sso.StateStore`, which `cache.Cache` satisfies structurally) and the composition root (`internal/app`) injects the concrete backend from a two-switch matrix: `cache_enabled` (default true) governs the content cache on the public read paths (off → direct database reads; on + valkey off → memory backend; on + valkey on → the shared valkey connection), while `valkey_enabled`/`valkey_url` govern the distributed state store behind rate limiting and SSO. Without valkey every consumer keeps its per-process implementation; with valkey, startup dials and PINGs the server (fail-fast on misconfiguration), and runtime store errors fail open in the rate limiter but fail closed in the SSO state check.
