...
> 配置 VexGo 的全部方式：命令行参数、环境变量和配置文件键。操作示例见[配置指南](/zh-cn/guides/configuration)。
...
使用 `./vexgo server` 启动服务器。不带子命令运行 `./vexgo` 会打印帮助信息而不是启动服务器。服务器参数必须跟在 `server` 之后；不接受位置参数。

运行 `./vexgo server --help` 获取服务器参数的权威列表。使用 `./vexgo --version`（或 `-V`）打印版本。

服务器参数（用法为 `vexgo server <参数>...`）：
